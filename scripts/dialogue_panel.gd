...
const FONT_DIALOGUE := preload("res://assets/fonts/ClimateCrisisKR-2040.ttf")   # 대사 본문 전용(Climate Crisis 2번째 얇은 두께)
...
static func build(parent: Node, vp: Vector2, portrait: Texture2D = null) -> Dictionary:
	...
	text_lbl.add_theme_font_override("font", FONT_DIALOGUE)     # 대사 본문 = Climate Crisis 2040(2번째 얇음)
