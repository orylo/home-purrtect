...
## 전리품·보석 도감 노출 설명(인벤토리/클리어 화면 탭 시 툴팁). 문구 출처=기획_전리품보석도감.md(노출 문구만, 〔내부용〕 제외).
## ★도감과 같은 커밋에서 함께 고친다(개발원칙 7). 두 줄 구성은 \n으로 보존.
const MAT_DESC := {
	"fur_gray":        "졸병 쥐들이 우르르 몰려오다 흘리고 간 까칠한 잿빛 털 뭉치.\n값은 헐하지만, 솜씨 좋은 손에 들어가면 제법 쓸모가 있다.",
	"fur_black":       "한 수 위 고참 쥐의 뻣뻣하고 윤기 도는 검은 털.\n회색 털보다 손맛이 좋아 값도 조금 더 쳐준다.",
	"wheel":           "롤러쥐의 스케이트에서 떨어져 나온 작은 쇠바퀴. 아직도 데구루루 잘 구른다.\n고물이라 헐값이지만, 굴러가는 데는 쓸모 있다.",
	"sack":            "투척쥐가 돌을 담아 끌고 다니던 거친 자루의 찢긴 조각.\n천이라 값은 헐하지만 질기다.",
	"bat_wing":        "처마 밑 훼방꾼이 떨군 얇고 가벼운 막 날개.\n가볍기만 하고 값은 영 안 나간다.",
	"sparrow_feather": "급강하하던 날쌘돌이가 남기고 간 보드라운 깃털 한 줌.\n헐값이지만 만지면 기분은 좋다.",
	"honey_drop":      "침입자 벌이 떨군 끈적하고 달콤한 황금빛 꿀 한 방울.\n달콤한 만큼 다른 잡템보단 조금 값이 나간다.",
	"spider_silk":     "매복꾼 거미가 뽑아내던 질기고 끈끈한 은빛 실.\n값은 별로라도 무언가 묶는 데는 으뜸이다.",
	"cheese_crumb":    "회색쥐가 노리던 치즈에서 떨어진 자잘한 부스러기. 고소한 냄새가 솔솔 난다.\n가장 흔하고 가장 헐하지만, 청소부에겐 요긴하다.",
	"cheese":          "검은쥐가 야무지게 챙겨가던 제법 큼직한 치즈 한 덩이.\n부스러기보다 값이 나간다.",
	"nail":            "롤러쥐 스케이트에 박혀 있던 구부정하고 녹슨 못 하나.\n고물이지만 악기를 손볼 땐 이만한 게 없다.",
	"button":          "어디선가 떨어져 나온 낡은 단추. 쥐도 거미도 어쩐 일인지 곧잘 물고 다닌다.",
	"thread_spool":    "투척쥐가 포대를 꿰매던 실이 칭칭 감긴 작은 나무 실패.",
	"safety_pin":      "검은 투척쥐가 자루를 여미는 데 쓰던, 작게 반짝이는 옷핀.",
	"cotton":          "박쥐 둥지에서 빠져나온 폭신폭신한 솜 한 덩이.\n가볍고 값도 가볍다.",
	"bread":           "참새가 물고 다니던, 딱딱하게 굳은 빵 한 조각.",
	"honeycomb":       "침입자 벌이 짓다 만 육각형 밀랍 벌집 조각. 달콤한 향이 배어 있다.\n잡템치곤 제법 값이 나간다.",
	"gem_gravel":      "길바닥에 흔히 굴러다니는 작은 돌멩이. 그래도 반들반들하다.\n헐값이지만, 누군가는 이런 사소한 것에도 살며시 마음을 열지 모른다.",
	"gem_pebble":      "물가에서 곱게 닳은 동글동글한 돌. 손에 쥐면 어쩐지 기분이 좋다.\n큰 값은 못 되지만, 작은 정성으로 건네기엔 나쁘지 않다.",
	"gem_shell":       "바닷바람 냄새가 밴 무지갯빛 조개껍데기.\n귀에 대면 파도 소리가 들린다나. 선물하면 살짝 웃어줄지도 모른다.",
	"gem_marble":      "햇빛에 비추면 알록달록 빛나는 작은 유리알.\n헐값은 겨우 면했다. 누군가의 주머니에 살짝 넣어주면 좋아할 것 같다.",
	"gem_glass_bead":  "티 없이 맑은 유리구슬. 들여다보면 세상이 거꾸로 담긴다.\n맑은 만큼 마음도 비쳐 보일까. 건네면 한참을 들여다볼 것 같다.",
	"gem_agate":       "은은한 줄무늬가 감도는 매끈한 돌. 제법 손맛이 난다.\n이쯤 되면 푼돈은 넘는다. 선물로 내밀어도 모양새가 산다.",
	"gem_quartz":      "거칠게 깎인 채로도 영롱하게 빛나는 수정 덩어리.\n값이 쏠쏠하다. 바치면 제법 기뻐할 만하다.",
	"gem_amber":       "오랜 세월 굳은 황금빛 송진. 안에 작은 벌레가 갇혀 있기도 하다.\n값도 쏠쏠하고, 선물하면 신기해하며 들여다볼 것이다.",
	"gem_amethyst":    "영롱한 보랏빛 결정. 가만히 들여다보면 마음이 차분해진다.\n제법 쏠쏠한 값이지만, 누군가에게 선물하면 마음을 얻을 수 있을 것 같다.",
	"gem_garnet":      "잘 익은 석류알처럼 붉게 타오르는 보석.\n팔면 쏠쏠하지만, 이 정도 귀한 걸 건네면 마음이 크게 흔들릴 것이다.",
	"gem_rose":        "은은한 분홍빛이 감도는 사랑스러운 수정.\n한밑천 잡을 값이지만, 이 빛깔이라면 파는 게 도리어 아깝다.",
	"gem_teal":        "맑은 하늘과 바다를 한데 섞은 듯한 청록빛 돌.\n한밑천이 되는 귀물. 이런 걸 건네면 두고두고 기억될 것이다.",
	"gem_sapphire":    "깊은 밤하늘을 그대로 담은 듯한 새파란 보석.\n큰돈이 되지만, 이런 걸 선뜻 내미는 마음은 돈으로 살 수 없다.",
	"gem_emerald":     "싱그러운 초록빛이 깊고 영롱하게 빛나는 보석.\n한밑천을 훌쩍 넘는 값. 바치면 눈이 휘둥그레질 것이다.",
	"gem_pearl":       "은은한 빛을 머금은 둥근 진주. 흠 하나 없이 곱다.\n귀물 중의 귀물. 이런 걸 받고 마음이 안 흔들릴 이가 있을까.",
	"gem_teardrop":    "한 방울 눈물이 그대로 굳은 듯, 맑고도 애틋하게 빛나는 수정.\n값을 매기기 아까운 귀물. 건네는 순간 진심이 그대로 전해질 것이다.",
	"gem_ruby":        "타오르는 불꽃을 가둔 듯 새빨간 보석. 보기만 해도 가슴이 뛴다.\n이쯤 되면 보물. 평생 단 한 번의 선물로 삼아도 모자라지 않는다.",
	"gem_diamond":     "세상 무엇보다 단단하고 눈부신 결정. 전설 속에서나 보던 보물.\n팔면 한 재산이지만, 이걸 누군가에게 바친다면 그건 곧 마음 전부를 건네는 일이다.",
}
